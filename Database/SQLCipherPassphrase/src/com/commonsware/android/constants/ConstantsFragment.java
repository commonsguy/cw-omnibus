		...
		DialogInterface.OnClickListener
{
	...
	public void onDestroy()
	{
		...
		getCursorAdapter().getCursor().close();
		db.close();
	}
	...
	private class LoadCursorTask extends AsyncTask<Void, Void, Void>
	{
		...
		public void onPostExecute(Void arg0)
		{
			...
			String[] fields = new String[] { DatabaseHelper.TITLE, DatabaseHelper.VALUE };
			int[] ids = new int[] { R.id.title, R.id.value };
			
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
			{
				adapter = new SimpleCursorAdapter(getActivity(), R.layout.row,
						constantsCursor, fields, ids, 0);
			}
			else
			{
				adapter = new SimpleCursorAdapter(getActivity(), R.layout.row,
						constantsCursor, fields, ids );
			}
	...
	private class InsertTask extends AsyncTask<ContentValues, Void, Void>
	{
		...
		public void onPostExecute(Void arg0)
		{
			getCursorAdapter().changeCursor(constantsCursor);
		}
	}
}
