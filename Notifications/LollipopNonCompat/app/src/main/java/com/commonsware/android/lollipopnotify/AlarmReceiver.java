...
public class AlarmReceiver extends BroadcastReceiver {
  ...
  private void notifyPrivate(Context ctxt, NotificationManager mgr) {
    Notification pub=buildPublic(ctxt).build();

    mgr.notify(NOTIFY_ID, buildNormal(ctxt).setPublicVersion(pub).build());
  }

  private void notifyPublic(Context ctxt, NotificationManager mgr) {
  ...
  private void notifySecret(Context ctxt, NotificationManager mgr) {
    mgr.notify(NOTIFY_ID,
        buildNormal(ctxt)
  ...
  private void notifyHeadsUp(Context ctxt, NotificationManager mgr) {
    mgr.notify(NOTIFY_ID,
        buildNormal(ctxt)
  ...
  private Notification.Builder buildNormal(Context ctxt) {
    Notification.Builder b=new Notification.Builder(ctxt);
  ...
  private Notification.Builder buildPublic(Context ctxt) {
    Notification.Builder b=new Notification.Builder(ctxt);
