...
public class RestrictionEntriesReceiver extends BroadcastReceiver
{
	...
	{
		Bundle current = (Bundle) intent
				.getParcelableExtra(Intent.EXTRA_RESTRICTIONS_BUNDLE);
	...
	private RestrictionEntry buildBooleanRestriction(Context ctxt,
			Bundle current)
	{
		RestrictionEntry entry = new RestrictionEntry(RESTRICTION_BOOLEAN,
				current.getBoolean(RESTRICTION_BOOLEAN, false));
	...
	{
		RestrictionEntry entry = new RestrictionEntry(RESTRICTION_CHOICE,
	...
	private RestrictionEntry buildMultiSelectRestriction(Context ctxt,
			Bundle current)
	{
		RestrictionEntry entry = new RestrictionEntry(RESTRICTION_MULTI,
