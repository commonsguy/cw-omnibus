...
public class SettingsSetter extends ListActivity
{
	...
	static
	{
		menuActivities.put(R.id.app, Settings.ACTION_APPLICATION_SETTINGS);
		menuActivities.put(R.id.security, Settings.ACTION_SECURITY_SETTINGS);
		menuActivities.put(R.id.wireless, Settings.ACTION_WIRELESS_SETTINGS);
		menuActivities.put(R.id.all, Settings.ACTION_SETTINGS);

		settings.add(new BooleanSetting(
				Settings.System.INSTALL_NON_MARKET_APPS,
				"Allow non-Market app installs", true));
		settings.add(new BooleanSetting(
				Settings.System.HAPTIC_FEEDBACK_ENABLED, "Use haptic feedback",
				false));
	...
	protected void onListItemClick(ListView l, View v, int position, long id)
	{
		...
		s.setChecked(getContentResolver(), l.isItemChecked(position));
	}
	...
	static class BooleanSetting
	{
		...
		BooleanSetting(String key, String displayName, boolean isSecure)
		{
			this.key = key;
